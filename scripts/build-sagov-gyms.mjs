...
if (!inputPath) {
  console.error(
    "Usage: node scripts/build-sagov-gyms.mjs <sagov-venues.csv> --output <source-package.json> [--existing <gyms.json>] [--fetched-at <YYYY-MM-DD>]"
  );
  process.exit(1);
}
...
try {
  ...
  const result = buildSagovGymSourcePackage(parseCsv(csvText), { fetchedAt, existingGyms });
  ...
  console.error("Findgym sa.gov sports facility source build");
...
function printSummary(result, outputPath) {
  console.log("Findgym sa.gov sports facility source build");
